        ...
        this.action({
            ...
            handler: context => {
                ...
                if(topCard.getType() === 'joker') {
                    ...
                    this.game.resolveGameAction(GameActions.aceCard({ card: topCard }), context);
                ...
                topCard = opponent.drawDeck[0]; // check if there is
                const topDiscard = opponent.discardPile.length > 0 ? opponent.discardPile[0] : null;
                if(topDiscard && (topCard.suit.toLowerCase() === topDiscard.suit.toLowerCase() ||
                    topCard.value === topDiscard.value)) {
